...
This chapter describes the methodology used to conduct the study of the state of the practice for MPS software and addresses RQ1 (software candidate identification) by documenting how the candidate set was assembled. The methodology follows \citet{Smith2021} and adapts it to the MPS domain. It proceeds in the following major steps, which map to the sections of this chapter:

\begin{enumerate}
    \item \textbf{Select the research domain} (Section~\ref{sec:domain-selection}).
    \item \textbf{Engage a domain expert} who participates in the assessment and later reviews the assembled candidate list and filtering outcome (Section~\ref{sec:domain-expert-interaction}).
    ...
    \item \textbf{Filter the ranked list} to the selected set of 28 packages, which the domain expert then reviews (Sections~\ref{sec:filtering-criteria} and~\ref{sec:final-software-set}).
...
The primary activity indicator used in this report is the Domain~X alive/dead rule: a package is considered alive if it has received commits or version releases within the last 18 months from the measurement date. This indicator was used as a weighed signal rather than as an automatic cutoff: a package was set aside on activity grounds only when prolonged inactivity coincided with the tool no longer being prominent or having been superseded by an actively maintained alternative. On that basis, the following packages were excluded:
...
\end{tikzpicture}
\caption{Candidate selection pipeline, from the three discovery sources to the 28 measured packages. The activity and maintenance criterion is a weighed signal rather than a hard cutoff: packages are set aside on activity grounds only when they are both inactive and no longer prominent, while inactive but still-prominent packages are retained and their alive/dead status is reported as a measured outcome in Chapter~\ref{ch:measurement-results}.}
...
\caption{Final software set: primary role, repository release date, last commit date, and a representative publication. Repository dates are a May~2026 snapshot; the ``Released'' date is the first release in the measured public repository, which for packages renamed, re-licensed, or migrated to open source after an earlier proprietary or pre-release phase (CoppeliaSim, MuJoCo, Webots, Gazebo, PhysX) reflects that later event rather than the project's original creation. ``---'' marks packages for which no representative publication was selected; the listed publications are the most representative reference available and are not necessarily peer reviewed.}
