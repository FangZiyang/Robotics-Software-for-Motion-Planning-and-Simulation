...
In this report, AHP is applied to the nine quality impression scores for the 28 packages. Because no criterion-weight elicitation was carried out, the qualities are weighted equally ($c_k = 1/9$); the overall priority of Equation~\eqref{eq:ahp-aggregate} is therefore the mean of the nine per-quality AHP priorities, not the arithmetic mean of the original impression scores. Chapter~\ref{ch:measurement-results} uses the resulting AHP ranking as its combined ranking and retains the arithmetic mean only as a descriptive comparison. The robustness check perturbs each impression score by up to $\pm 10\%$ and recomputes the AHP ranking, following the LBM assessment~\citep{Smith2024LBM}.
