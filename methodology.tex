...
LLM-generated software lists were used as an auxiliary source for candidate discovery. This modifies the original Domain~X methodology, which prescribes search engine queries and domain expert consultation as the primary discovery methods. The exact model versions and prompts used for this step were not recorded, so the LLM queries themselves cannot be reproduced; this is a limitation of the discovery process. Its effect is contained because the LLM output served only as an auxiliary source: every name it surfaced was consolidated with the academic and community-curated lists and verified against public evidence before it informed any claim in this report.

The reason for including LLM-assisted discovery is practical. Large language models surface software names that appear across a wide range of public information. LLM output can also be incomplete, outdated, or incorrect, so names obtained from LLM-generated lists were consolidated, filtered, and verified against public evidence before any claims in this report were based on them.

The use of LLMs is defensible for two reasons: (i)~LLMs were used together with academic and community-curated sources, not as the sole discovery method; and (ii)~an LLM, in this context, is a large-scale aggregator over existing public information, analogous in effect to a broad search over indexed web content.
...
By contrast, inactivity was not treated as grounds for exclusion for packages that remain prominent and widely referenced in the MPS domain. Four such packages (OpenRAVE, MORSE, GraspIt!, and Flightmare) had become inactive by the measurement date but were retained in the measured set; their alive/dead status is recorded and reported as a measured outcome in Chapter~\ref{ch:measurement-results} (Section~\ref{sec:repository-project-metadata}), so that the study can examine how software quality persists after development activity declines.
...
The domain expert, Dr.~Matthew Giamou, was involved in selecting the software packages: he reviewed the candidate list and the filtering outcome, confirmed that the resulting set was reasonable and that most of the packages were familiar to him, and noted that ROS~2, MoveIt!, and OMPL are not full simulators but fit within a coherent narrative when their roles are explained carefully. He agreed that excluding MATLAB/Simulink, Unity, NVIDIA Isaac Sim, and RaiSim on open-source grounds (Section~\ref{subsec:open-source-availability}) would not misrepresent the MPS domain; MRDS and USARSim were filtered on activity grounds (Section~\ref{subsec:activity-maintenance-status}) rather than by a separate expert decision.
...
The role column is necessary because the packages are not direct substitutes; Chapter~\ref{ch:selected-software-commonality} provides the detailed category and commonality analysis used to interpret the boundary cases such as ROS~2, MoveIt!, and OMPL.

\section{Measurement Procedure}
\label{sec:measurement-procedure}

The measurement procedure follows the Domain~X method for collecting structured evidence about software packages in a selected domain~\citep{Smith2021}. For each of the 28 selected packages, the study gathered source code, documentation, publications where applicable, repository metadata, issue-tracker evidence, release information, and installation evidence. The measurement record identifies the specific repository or repositories measured (Section~\ref{subsec:repository-based-measurability}).
...
Because the selected packages span multiple roles, the commonality analysis does not force all packages into a single comparison template. Its purpose is to describe shared concerns and meaningful differences, not to claim that every package provides identical functionality. The analysis is based on checked evidence from documentation, repositories, publications, and other reliable project materials. Capabilities that could not be verified against these materials are not reported.
