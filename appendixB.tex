...
The four GitHub Topic pages are retained because they are recorded in the source catalogue (\path{examples/sofware-counting-webs.csv}). They are dynamic GitHub aggregations, so copied page text was not preserved in \path{examples/web-list-raw-material.md} in the same way as the static curated-list pages.
...
\midrule
Brugali et al. (2007), ``Trends in Robotic Software Frameworks'' (book chapter) & ROS, OROCOS, OpenRTM-aist, Gazebo, YARP \\
...
The source catalogue with community-source URLs and archived LLM-assisted outputs is preserved in \path{examples/sofware-counting-webs.csv}. Raw captured text from static public web lists is preserved in \path{examples/web-list-raw-material.md}. The consolidated ranking with per-package mention counts is in \path{examples/software-list-counting-result.xlsx} (sheet \texttt{final.m}).
