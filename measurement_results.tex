...
% TODO (sensitivity): the 10\% score-perturbation check described in Section~\ref{sec:ahp-background} is run separately; once the result is available, add one sentence here reporting its effect on the ranking (e.g. whether the top tier is unchanged), so Chapter 5 actually delivers the sensitivity check that the method describes.
