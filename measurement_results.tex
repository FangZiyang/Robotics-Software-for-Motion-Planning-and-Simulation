...
As noted in Section~\ref{sec:installability-results}, installation broke for 2 of 28 packages (OpenRAVE and YARP). In both cases the installation instance was recoverable; for OpenRAVE a descriptive error message was displayed, while the measurement record does not capture the error output for YARP.
...
Table~\ref{tab:overall-impressions} summarizes the overall impression scores across all nine quality categories for each package, along with a simple unweighted per-package mean. Figure~\ref{fig:quality-ranking} plots the per-package mean as a ranked bar chart coloured by activity status, Figure~\ref{fig:quality-heatmap} renders the full score matrix as a heatmap, and Figure~\ref{fig:category-distribution} shows how the scores are distributed within each of the nine categories. The scores reported here are the raw 1--10 impression values assigned during measurement using the Domain~X Appendix~C calculator. The unweighted mean treats all nine qualities as equally important. Because this study does not elicit criterion weights, Figure~\ref{fig:ahp-ranking} presents an illustrative AHP ranking under equal category weights ($c_k = 1/9$), following the pairwise-comparison, priority, and aggregation steps defined in Section~\ref{sec:ahp-background} (Equations~\eqref{eq:ahp-pairwise}--\eqref{eq:ahp-aggregate}). Under equal weights the AHP global priorities and the unweighted mean agree at the top of the table, as Equation~\eqref{eq:ahp-aggregate} implies.
...
\begin{figure}[htbp]
    ...
    \caption{Distribution of the impression scores across the 28 packages within each of the nine quality categories (Table~\ref{tab:overall-impressions}). Boxes span the interquartile range with the median marked; whiskers and points show the spread and outliers.}
...
\begin{figure}[htbp]
    ...
    \caption{Illustrative equal-weight AHP global-priority ranking ($c_k = 1/9$), computed from the overall impression scores of Table~\ref{tab:overall-impressions} by the AHP procedure of Section~\ref{sec:ahp-background} (priorities sum to one; higher is better).}
    \label{fig:ahp-ranking}
\end{figure}

To check that this ordering does not hinge on the precise impression scores, the AHP ranking was recomputed under a Monte~Carlo perturbation of its inputs. Each of the 252 score entries in Table~\ref{tab:overall-impressions} was independently scaled by a factor drawn uniformly from $[0.9, 1.1]$, a $\pm 10\%$ perturbation clipped to the 1--10 range, and the equal-weight global priorities were re-derived over $1{,}000$ trials with a fixed random seed, using the May~2026 score snapshot. The overall ordering is well preserved: the mean Spearman rank correlation with the baseline ranking is $0.97$ (minimum $0.87$ over the trials) and the mean Kendall~$\tau$ is $0.88$ (minimum $0.74$). Gazebo remains the top-ranked package in every trial, and the two lowest-ranked packages, Newton~Dynamics and GraspIt!, are likewise fixed. The five highest-scoring packages after Gazebo (Drake, MoveIt!, Pinocchio, ROS~2, and OMPL) never leave the top eight positions, but they are interchangeable within that band, with Drake alone ranging over ranks 2--6 (Figure~\ref{fig:ahp-sensitivity}). The exact ordering among them should therefore not be over-interpreted: the precise top-three and top-five sets recur in only $22\%$ and $26\%$ of trials, respectively, because these packages are separated by score gaps smaller than the perturbation. The largest movement, up to ten positions, occurs in the densely packed middle of the table, where many packages share nearly identical scores. The check thus supports the coarse structure of the ranking, namely a clear leader, a stable leading tier, a closely packed middle, and a fixed bottom, while reinforcing that the equal-weight AHP ordering is an illustrative analysis aid rather than a precise verdict on individual positions (Section~\ref{sec:ahp-background}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/fig_ahp_sensitivity}
    \caption{Stability of the equal-weight AHP ranking under a $\pm 10\%$ score perturbation (May~2026 scores). Diamond, baseline rank; thick bar, 5th--95th percentile; thin line, full range.}
...
\begin{figure}[htbp]
    ...
    \caption{Coverage of selected measurement-template indicators across the 28 packages (May 2026), sorted from least to most common. Values restate Tables~\ref{tab:installability}, \ref{tab:correctness-summary}, \ref{tab:maintainability-summary}, \ref{tab:understandability-summary}, and \ref{tab:visibility-summary} and the documented-user-characteristics result of Section~\ref{sec:surface-usability-results}.}
...
Chapter~\ref{ch:comparison-research-software} interprets these results against prior studies of the state of the practice, and Chapter~\ref{ch:conclusions} answers the research questions in light of the evidence.
