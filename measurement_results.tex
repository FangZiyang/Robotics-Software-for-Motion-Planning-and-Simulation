...
The Domain~X quality mean (Section~\ref{sec:overall-observations}) reflects per-package documentation, testing, and release-management practice, while community indicators such as GitHub stars, forks, and recent commit activity capture a different signal: cumulative visibility and adoption built up over time. The two need not agree. This section presents both rankings side by side, using the per-package quality mean from Table~\ref{tab:overall-impressions} and the star count from Table~\ref{tab:repository-metrics}, and identifies where they converge and diverge.
