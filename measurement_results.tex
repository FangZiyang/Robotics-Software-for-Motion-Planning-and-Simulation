...
Table~\ref{tab:correctness-summary} summarizes key correctness and verifiability indicators (Section~\ref{sec:software-quality-attributes}).
...
Surface reliability (Section~\ref{sec:software-quality-attributes}) covers only the interactions performed during measurement: installation and initial tutorial use.
...
Surface usability (Section~\ref{sec:software-quality-attributes}) covers user-facing documentation, tutorials, and support infrastructure; no user studies or task-completion experiments were conducted.
...
Table~\ref{tab:maintainability-summary} summarizes key maintainability indicators (Section~\ref{sec:software-quality-attributes}).
...
Reusability (Section~\ref{sec:software-quality-attributes}) is measured through two indicators: the number of code files (a rough proxy for component granularity) and the presence of API documentation.
...
Surface understandability (Section~\ref{sec:software-quality-attributes}) was assessed through a review of 10 randomly selected source files per package. One template question, whether parameters appear in the same order across functions, was not recorded during measurement; the discussion and scores below rest on the remaining checks.
...
Table~\ref{tab:visibility-summary} summarizes the key visibility and transparency indicators (Section~\ref{sec:software-quality-attributes}).
...
Against the 10\% comment-density threshold used by the maintainability calculator (Section~\ref{sec:maintainability-results}), 15 of the 27 packages with comparable measurements (about 56\%) exceed it. The packages below 10\% include some of the largest codebases in the study (Bullet/PyBullet at 2.8\%, SOFA at 4.5\%, Project Chrono at 1.1\%), so codebase size alone does not determine comment density.
...
\begin{figure}[htbp]
    ...
    \caption{Quality-mean ranking of the 28 packages (unweighted mean of the nine category scores), coloured by activity status.}
...
The relationship between community popularity and measurement scores is not linear; Section~\ref{sec:quality-vs-community} reports the explicit ranking comparison.
...
\begin{figure}[htbp]
    ...
    \caption{Measured quality mean versus GitHub stars (log scale) for the 28 packages, coloured by activity status. Dashed lines mark the mean quality and the median star count.}
