...
This chapter characterizes the final 28 packages (Section~\ref{sec:final-software-set}) for RQ1 and gives the commonality context used later when interpreting the RQ2 measurements. It summarizes the packages, groups them by primary role, identifies shared capabilities and major variation points, records parameters of variation and binding times, and closes with boundary cases that do not fit cleanly into one role. The commonality analysis follows the \citet{Weiss1998} procedure described in Section~\ref{sec:commonality-analysis-procedure}; capabilities are reported only where they could be verified against the project materials.

\section{Overview of Selected Software}
\label{sec:overview-selected-software}

The selected packages come from a mix of development settings. Academic research groups account for several entries (for example, Drake, DART, and OMPL). Open-source foundations and companies steward others, notably ROS~2 and Gazebo through Open Robotics, with the ROS-Industrial Consortium coordinating related industrial applications. Corporate research labs contribute MuJoCo (Google DeepMind), AirSim (Microsoft Research), and Habitat (Meta AI Research). Smaller communities or individual contributors maintain projects such as ODE and Newton Dynamics.
...
Measurement scores and quality rankings are not reported here; those appear in Chapter~\ref{ch:measurement-results}.

\section{Functional Software Roles}
\label{sec:robotics-software-categories}

The selected packages can be grouped into three broad functional roles by their primary function in a robotics workflow. These roles are not strict boundaries---some packages span several---but they structure the commonality analysis that follows.
...
Spatial dimensionality was considered as a possible variability but is not listed among V1--V9: every selected package operates in three dimensions, and no purely 2D or planar-only tool appears in the final set. The one 2D-capable candidate, Player/Stage, was not selected (Appendix~\ref{appendix:full-candidate-software-list}).
...
The binding times reflect when a user or integrator can still make a choice. Primary role and governance are inherent to a package's design and so are fixed at specification time, whereas ROS integration depth is often a configuration-time choice because many packages offer optional bridges, and physics fidelity can be bound at both specification time (choice of engine) and run time (solver parameters or time step).

\section{Boundary Cases}
\label{sec:boundary-cases}

Several of the selected packages occupy intermediate positions in the classification above. These boundary cases are not classification errors; they illustrate the interconnected nature of the robotics software ecosystem and the difficulty of drawing sharp boundaries between roles.
