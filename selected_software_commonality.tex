...
\begin{itemize}
    ...
    \item \textbf{AirSim} is an open-source simulator for autonomous vehicles (drones and ground vehicles) built on Unreal Engine and developed by Microsoft Research. The measured repository is \texttt{microsoft/AirSim}. Its last commit (2026-03-15) produces an alive value under the mechanical 18-month repository-activity flag defined in Section~\ref{subsec:activity-maintenance-status}. However, the README has stated since 2022 that development has ended (``no further updates will be made, effective immediately'') and directs users to Project AirSim; the later commits are README-level changes. AirSim is therefore treated as officially discontinued despite its alive repository-activity flag.
