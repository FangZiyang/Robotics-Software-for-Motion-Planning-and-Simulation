...
\begin{table}[htbp]
\centering
\caption{Cross-domain comparison of repository artifacts, tools, and process evidence: motion planning and simulation (MPS, this study) against the medical imaging (MI) and Lattice Boltzmann method (LBM) State-of-the-Practice studies. MPS counts are out of 28 packages (Chapter~\ref{ch:measurement-results}, May 2026); MI counts are out of 29 packages~\citep{Smith2024MI} and LBM counts out of 24 packages~\citep{Smith2024LBM}. Percentages are taken over each study's own package count, so the columns are comparable despite the different denominators.}
